...
class EVENTBPIC17(InMemoryDataset):
    
    url = 'https://github.com/keyvan-amiri/PGTNet/raw/main/transformation/bpi17_graph_raw.zip'  

    def __init__(self, root, split='train', transform=None, pre_transform=None,
                 pre_filter=None):
        self.name = "EVENTBPIC17"
        assert split in ['train', 'val', 'test']
        super().__init__(root, transform, pre_transform, pre_filter)
        path = osp.join(self.processed_dir, f'{split}.pt')
        self.data, self.slices = torch.load(path)
    ...
    def download(self):
        ...
        path = download_url(self.url, self.root)
        extract_zip(path, self.root)
        os.rename(osp.join(self.root, 'bpi17_graph_raw'), self.raw_dir)
        os.unlink(path)
    ...
    def process(self):
        for split in ['train', 'val', 'test']:
            ...
            for idx in indices:
                ...
                """
                Each `graph` is a Pytorch Geometrioc Data object:
                    Shape of x : [num_nodes, 1] any integer from 0 to 31 represents one type of activity
                    class i.e. unique (activity identifier, lifecycle transition) tuples similar to
                    different types of atoms.
                    Shape of edge_attr : [num_edges, 187] the edge attributes represent weight of Directly
                    Followed realationships, temporal features, number of active cases in the process,
                    as well as data perspective in both case and event level.                     
                    Shape of edge_index : [2, num_edges] edge index determines topology of the graph. Our
                    main assumption is that process variants can be expressively represented by type of
                    ...
                    Shape of y : [1] a number between zero and one if normalized. You are more than
                    welcome to explore possibilities for learning without normalization. But, here:
                    normalizization coeficient = 169.031 (days)
                """
        ...
        for split in ['train', 'val', 'test']:
            ...
            for idx in indices:
                ...
                Each `graph` is a Pytorch Geometrioc Data object:
                    ...
                    class i.e. unique (activity identifier, lifecycle transition) tuples similar to
                    different types of atoms.
                    Shape of edge_attr : [num_edges, 262] the edge attributes represent weight of Directly
                    Followed realationships, temporal features, number of active cases in the process,
                    as well as data perspective in both case and event level.                     
                    Shape of edge_index : [2, num_edges] edge index determines topology of the graph. Our
                    main assumption is that process variants can be expressively represented by type of
...
class EVENTBPIC20P(InMemoryDataset):
   
    url = 'https://github.com/keyvan-amiri/PGTNet/raw/main/transformation/bpi20p_graph_raw.zip'    

    def __init__(self, root, split='train', transform=None, pre_transform=None,
                 pre_filter=None):
        self.name = "EVENTBPIC20P"
        assert split in ['train', 'val', 'test']
        super().__init__(root, transform, pre_transform, pre_filter)
        path = osp.join(self.processed_dir, f'{split}.pt')
        self.data, self.slices = torch.load(path)
    ...
    def process(self):
        for split in ['train', 'val', 'test']:
            ...
            for idx in indices:
                ...
                """
                Each `graph` is a Pytorch Geometrioc Data object:
                    Shape of x : [num_nodes, 1] any integer from 0 to 28 represents one type of activity
                    class i.e. unique (activity identifier, lifecycle transition) tuples similar to
                    different types of atoms.
                    Shape of edge_attr : [num_edges, 206] the edge attributes represent weight of Directly
                    Followed realationships, temporal features, number of active cases in the process,
                    as well as data perspective in both case and event level.                     
                    Shape of edge_index : [2, num_edges] edge index determines topology of the graph. Our
                    main assumption is that process variants can be expressively represented by type of
                    ...
                    Shape of y : [1] a number between zero and one if normalized. You are more than
                    welcome to explore possibilities for learning without normalization. But, here:
                    normalizization coeficient = 324.976 (days)
                """                 
                x = graph.x
                edge_attr = graph.edge_attr
                edge_index = graph.edge_index
        ...
        for split in ['train', 'val', 'test']:
            ...
            for idx in indices:
                ...
                Each `graph` is a Pytorch Geometrioc Data object:
                    ...
                    class i.e. unique (activity identifier, lifecycle transition) tuples similar to
                    different types of atoms.
                    Shape of edge_attr : [num_edges, 80] the edge attributes represent weight of Directly
                    Followed realationships, temporal features, number of active cases in the process,
                    as well as data perspective in both case and event level.                     
                    Shape of edge_index : [2, num_edges] edge index determines topology of the graph. Our
                    main assumption is that process variants can be expressively represented by type of
