...
from enable_estimates.utils import zip_with_next, EventLogIDs
...
class ConcurrencyOracle:
    ...
    def add_enabled_times(self, event_log: pd.DataFrame, set_nat_to_first_event: bool = False, include_enabling_activity: bool = True):
        """
        ...
        indexes, enabled_times, enabling_activities, enabling_resources = [], [], [], []
        for (case_id, trace) in event_log.groupby(self.log_ids.case):
            ...
            for index, event in trace.iterrows():
                ...
                if isinstance(enabling_activity_instance, pd.Series):
                    ...
                    enabling_resource_label = enabling_activity_instance[self.log_ids.resource]
                ...
                if include_enabling_activity:
                    ...
                    enabling_resources += [enabling_resource_label]
        ...
        if include_enabling_activity:
            ...
            event_log.loc[indexes, self.log_ids.enabling_resource] = enabling_resources           
