...
from graphgps.loader.dataset.GTeventlogHandler import (
    ...
    EVENTBPIC13I, EVENTBPIC13C, EVENTBPIC20D, EVENTBPIC20I, EVENTBPIC20P, EVENTBPIC17, 
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        elif pyg_dataset_id == 'EVENTBPIC20P':
            dataset = preformat_EVENTBPIC20P(dataset_dir)
...
def preformat_EVENTBPIC20P(dataset_dir):
    """Load and preformat EVENTBPIC20I datasets.
    Args:
        dataset_dir: path where to store the cached dataset
    Returns:
    ...
    """
    dataset = join_dataset_splits(
        [EVENTBPIC20P(root=dataset_dir, split=split)
         for split in ['train', 'val', 'test']]
    )
    return dataset
...
def preformat_EVENTBPIC17(dataset_dir):
    ...
    Returns:
        PyG dataset object
    """
    #logging.info(f"Loading dataset from {dataset_dir}...")  # Log to .out file
    dataset = join_dataset_splits(
        [EVENTBPIC17(root=dataset_dir, split=split)
         for split in ['train', 'val', 'test']]
    )
    #logging.info("Dataset loaded successfully.")  # Log to .out file
    """
    if dataset:
        current_max_value = 0
        for i, split_data in enumerate(dataset):
            node_features = split_data.x
            unique_values = torch.unique(node_features)
            max_value = unique_values.max().item()
            if max_value > current_max_value: 
                current_max_value = max_value  # Update the current maximum value
                logging.info(f"New max node value: {max_value}") 
    """
    return dataset
